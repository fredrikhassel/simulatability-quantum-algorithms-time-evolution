...
def batch_sampling(probs, batch_size, n_workers=None):
    if n_workers is not None:
        return mp.Pool(n_workers).map(sample_from_prob, [probs] * batch_size) 
    return mp.Pool(mp.cpu_count()-1).map(sample_from_prob, [probs] * batch_size)
